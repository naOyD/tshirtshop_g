 ...
 {section name=i loop=$obj->mCategories}
  {if $obj->mEditItem == $obj->mCategories[i].category_id}
   <tr>
    ...
    <td>
     <input type="submit"
     name="submit_edit_prod_{$obj->mCategories[i].category_id}" value="Edit Products"/>
     <input type="submit"
     name="submit_update_cat_{$obj->mCategories[i].category_id}" value="Update"/>
     <input type="submit" 
     name="Cancel" value="Cancel"/>
     <input type="submit" 
     name="submit_delete_cat_{$obj->mCategories[i].category_id}" value="Delete"/>
    </td>
   </tr>
  {else}
  <tr>
   ...
   <td>
    <input type="submit"
     name="submit_edit_prod_{$obj->mCategories[i].category_id}" value="Edit Products"/>
    <input type="submit"
    name="submit_edit_cat_{$obj->mCategories[i].category_id}" value="Edit"/>
    <input type="submit" 
    name="submit_delete_cat_{$obj->mCategories[i].category_id}" value="Delete"/>
   </td>
  </tr>
